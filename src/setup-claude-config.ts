  ...
  try {
    ...
    if (existsSync(CONFIG_FILE)) {
      try {
        ...
        console.log(
          "⚠️  Existing config file found but could not be parsed, will create new one",
        );
        existingConfig = {};
      }
    }
    ...
    console.log(
      "• share-memory, get-shared-memories, search-shared-memories-by-keywords",
    );
    console.log("");
    console.log(
      "💡 Tip: Set MONGODB_URI and SHARED_MONGODB_URI environment variables for custom database connections",
    );
  } catch (error) {
